...
#include "imgui_internal.h"  // 添加对imgui_internal.h的引用，以访问ImGuiContext的内部结构
...
struct ImGui_ImplGlfw_Data
{
    ...
    bool                    WindowIconified;  // 添加窗口最小化标志
    ...
    GLFWwindowiconifyfun    PrevUserCallbackWindowIconify;
#ifdef _WIN32
    WNDPROC                 PrevWndProc;
#endif
...
static void ImGui_ImplGlfw_WindowIconifyCallback(GLFWwindow* window, int iconified);
...
void ImGui_ImplGlfw_InstallCallbacks(GLFWwindow* window)
{
    ...
    bd->PrevUserCallbackWindowIconify = glfwSetWindowIconifyCallback(window, ImGui_ImplGlfw_WindowIconifyCallback);
...
void ImGui_ImplGlfw_RestoreCallbacks(GLFWwindow* window)
{
    ...
    glfwSetWindowIconifyCallback(window, bd->PrevUserCallbackWindowIconify);
...
void ImGui_ImplGlfw_Shutdown()
{
    ...
    if (bd->InstalledCallbacks && bd->Window != nullptr)
    {
        ...
        glfwSetMonitorCallback(bd->PrevUserCallbackMonitor);
        glfwSetWindowIconifyCallback(bd->Window, bd->PrevUserCallbackWindowIconify);
        bd->InstalledCallbacks = false;
    }
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    // 检查窗口是否最小化，如果是则设置DisplaySize为零以跳过渲染
    if (bd->WindowIconified)
    {
...
// 处理窗口最小化/还原事件
void ImGui_ImplGlfw_WindowIconifyCallback(GLFWwindow* window, int iconified)
{
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
    if (bd->PrevUserCallbackWindowIconify != nullptr && ImGui_ImplGlfw_ShouldChainCallback(window))
        bd->PrevUserCallbackWindowIconify(window, iconified);

    // 更新窗口最小化状态标志
    bd->WindowIconified = (iconified != 0);

    // 当窗口状态改变时，更新ImGui状态
    ImGuiIO& io = ImGui::GetIO();
    io.AddFocusEvent(!iconified);
    
    // 不再在回调中调用EndFrame/NewFrame，因为这可能破坏ImGui的帧同步
}
